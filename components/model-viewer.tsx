      ...
      <Html center>
        <div className="bg-[#262626] p-4 rounded-md text-white text-center">
          ...
          <p className="text-sm text-[#696969]">
            The model might be temporarily unavailable or still processing.
          </p>
        </div>
      </Html>
    );
...
export function ModelViewer({
  ...
  const [proxiedModelUrl, setProxiedModelUrl] = useState<string | undefined>(
    undefined
  );
  ...
  const handleModelError = useCallback(
    (error: Error) => {
      ...
      addToast({
        ...
        description:
          "There was a problem loading the 3D model. Please try again later.",
        duration: 5000,
      });
    },
    [addToast, onLoadEnd]
  ...
  if (
    ...
    const textureMap = textureUrls[0][
      currentView as keyof (typeof textureUrls)[0]
    ] as string;

    if (textureMap) {
      return (
        <div className="w-full h-full flex items-center justify-center bg-[#181818] p-4">
          <div className="relative max-w-full max-h-full">
            ...
            <div className="absolute bottom-4 left-4 bg-[#262626] px-3 py-1 rounded-md text-white text-sm">
              {currentView.replace("_", " ").charAt(0).toUpperCase() +
                currentView.replace("_", " ").slice(1)}{" "}
              Map
            </div>
          </div>
        </div>
  ...
  if (thumbnailUrl && !modelUrl) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-4">
        ...
        <p className="text-[#d1afe4] text-sm mt-2 text-center">
      ...
      {proxiedModelUrl && (
        <Canvas camera={{ position: [2, 2, 2], fov: 50 }}>
          <Suspense fallback={<LoadingSpinner progress={progress} />}>
            ...
            <Model
              url={proxiedModelUrl}
              wireframe={currentView === "wireframe"}
            />
            <OrbitControls
              enablePan={true}
              enableZoom={true}
