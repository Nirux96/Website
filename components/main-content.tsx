...
export function MainContent() {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const inProgress = sortedTasks.filter((task) => task.status === "PENDING" || task.status === "IN_PROGRESS")
  ...
  }, [fetchTasks, inProgressTasks.length])
  ...
  useEffect(() => {
    ...
    const handleTaskCreated = (event: CustomEvent) => {
          ...
          if (taskData && taskData.id) {
            ...
            setAllTasks((prev) => {
              ...
              if (exists) {
                ...
                return prev.map((task) => (task.id === taskData.id ? taskData : task))
              } else {
                // Add the new task
      ...
      const pollTaskStatus = async () => {
        try {
          ...
          if (data && data.id) {
            setAllTasks((prev) => prev.map((task) => (task.id === data.id ? data : task)))
            ...
            if (data.status === "PENDING" || data.status === "IN_PROGRESS") {
              ...
              setInProgressTasks((prev) => prev.filter((task) => task.id !== data.id))
            }
    ...
    return () => window.removeEventListener("taskCreated" as any, handleTaskCreated)
  ...
  const handleTexture = async (previewTask: Task) => {
    try {
      ...
      const event = new CustomEvent("taskCreated", {
        detail: { taskId: data.result, mode: "refine", previewTaskId: previewTask.id },
      })
      ...
      addToast({
        type: "error",
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to start texturing",
        duration: 3000,
      })
    } finally {
  ...
  const handleDownload = (url: string, format: string) => {
    ...
    try {
      ...
      addToast({
        ...
        description: "There was a problem downloading the model. Please try again.",
        duration: 3000,
      })
    }
  }
  ...
  const modelTasks = allTasks.filter((task) => task.mode === "preview" && task.status === "SUCCEEDED")

  // Filter tasks for the Textured section (refine mode)
  const texturedTasks = allTasks.filter((task) => task.mode === "refine" && task.status === "SUCCEEDED")

  // Update the return statement to pass the selected task to RightSidebar
  return (
    <div className="flex flex-1">
      {/* Left part - Task Listings */}
      <div className="w-1/2 bg-[#1e1e1e] relative flex flex-col">
        {/* Top bar */}
        <div className="p-4 border-b border-[#262626] flex justify-between">
          <div className="relative">
            ...
            <input
              type="text"
              className="pl-10 pr-4 py-2 bg-[#262626] text-white rounded-md w-64"
              placeholder="Search my generations"
            />
          </div>
          <div className="flex items-center">
            <button className="flex items-center px-3 py-1.5 bg-[#262626] text-white text-sm rounded-md mr-2">
              <span>Filters</span>
          ...
          {isLoading && allTasks.length === 0 ? (
            ...
            <>
              ...
              {inProgressTasks.length > 0 && (
                <div>
                  <h2 className="text-white text-sm font-semibold mb-4">In Progress</h2>
                  <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
                    {inProgressTasks.map((task) => (
                      <div
                        key={task.id}
                        className={`bg-[#262626] rounded-lg overflow-hidden transition-all hover:scale-[1.02] cursor-pointer ${
                          selectedTask?.id === task.id ? "ring-2 ring-[#d1afe4]" : ""
                        }`}
                        onClick={() => {
                        ...
                        <div className="h-32 bg-[#181818] flex items-center justify-center relative">
                          {task.thumbnail_url ? (
                            ...
                            <div className="flex flex-col items-center justify-center">
                              ...
                              <div className="text-white text-sm">{task.progress || 0}% Complete</div>
                            </div>
                          )}
                          {/* Progress bar overlay */}
                          <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#3f3f3f]">
                            <div className="h-full bg-[#d1afe4]" style={{ width: `${task.progress || 0}%` }}></div>
                          </div>
                        </div>
                        <div className="p-3">
                          <p className="text-white text-sm truncate mb-2">{task.prompt}</p>
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-xs px-2 py-0.5 rounded-full bg-purple-900/30 text-purple-400">
                              {task.mode === "preview" ? "Generating" : "Texturing"} ({task.progress || 0}%)
                            </span>
                          </div>
                        </div>
                      </div>
              ...
              {modelTasks.length > 0 && (
                <div>
                  <h2 className="text-white text-sm font-semibold mb-4">Models</h2>
                  <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
                    {modelTasks.map((task) => (
                      <div
                        key={task.id}
                        ...
                        <div className="p-3">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <button
                              ...
                              disabled={isTexturing || texturingTaskId === task.id}
                            >
                              {texturingTaskId === task.id ? "Texturing..." : "Texture"}
                            </button>
                          </div>
                        </div>
                      </div>
              ...
              {texturedTasks.length > 0 && (
                <div>
                  <h2 className="text-white text-sm font-semibold mb-4">Textured</h2>
                  <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {/* Bottom pagination - UI only, not functional */}
        <div className="p-4 border-t border-[#262626] flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex space-x-1">
              <button className="p-1 text-[#505050] disabled:opacity-50" disabled>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9 4L5 8L9 12" stroke="currentColor" strokeWidth="1.5" />
                </svg>
              </button>
              <button className="p-1 text-[#505050] disabled:opacity-50" disabled>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <button className="p-1 text-[#505050] disabled:opacity-50" disabled>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M7 4L11 8L7 12" stroke="currentColor" strokeWidth="1.5" />
                </svg>
              </button>
            </div>
          </div>
      ...
      <div className="w-1/2 bg-[#181818] relative">
        {/* Model viewer */}
          ...
          <div className="absolute bottom-4 right-4 flex space-x-2">
            {selectedTask.model_urls.glb && (
              <button
                ...
                onClick={() => handleDownload(selectedTask.model_urls.glb!, "glb")}
              >
            ...
            {selectedTask.model_urls.fbx && (
              <button
                className="px-3 py-1.5 bg-[#262626] text-white text-sm rounded-md flex items-center hover:bg-[#3f3f3f]"
                onClick={() => handleDownload(selectedTask.model_urls.fbx!, "fbx")}
              >
                <Download className="h-4 w-4 mr-1" />
                FBX
              </button>
            ...
            {selectedTask.model_urls.obj && (
              <button
                className="px-3 py-1.5 bg-[#262626] text-white text-sm rounded-md flex items-center hover:bg-[#3f3f3f]"
                onClick={() => handleDownload(selectedTask.model_urls.obj!, "obj")}
              >
                <Download className="h-4 w-4 mr-1" />
                OBJ
              </button>
      ...
      {/* Right sidebar */}
      <RightSidebar selectedTask={selectedTask} onViewChange={setCurrentView} currentView={currentView} />
    </div>
  )
}
