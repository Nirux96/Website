...
export function Navbar() {
  ...
  return (
    <header className="flex items-center justify-between px-4 py-3 bg-[#181818] border-b border-[#262626]">
      ...
      <div className="flex items-center">
        ...
        <div className="relative group">
          <button
            className="flex items-center px-3 py-1 text-white text-sm bg-[#262626] rounded-md mr-6"
            onClick={() => {
              addToast({
                type: "info",
          ...
          >
            ...
            >
              <path d="M2 4H14M2 8H14M2 12H14" stroke="white" strokeWidth="1.5" />
            </svg>
            <span>Text to 3D</span>
            <ChevronDown className="ml-1 h-4 w-4" />
          </button>
          <div className="absolute hidden group-hover:block top-full left-0 mt-1 w-64 bg-[#262626] rounded-md shadow-lg z-10">
            <div className="py-1">
              {WORKSPACE_OPTIONS.map((option) => (
                <div
                  key={option.id}
                  className={`flex items-start px-4 py-2 text-sm ${
                    option.comingSoon ? "opacity-50 cursor-not-allowed" : "hover:bg-[#3f3f3f] cursor-pointer"
                  }`}
                  onClick={() => {
                ...
                >
                  ...
                  <div>
                    <div className="text-white flex items-center">
                      {option.name}
                      {option.comingSoon && (
                        <span className="ml-2 text-xs bg-[#3f3f3f] px-2 py-0.5 rounded">Coming Soon</span>
                      )}
                    </div>
                    <div className="text-[#696969] text-xs mt-0.5">{option.description}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <nav className="hidden md:flex space-x-6">
          <a href="#" className="text-white text-sm hover:text-[#d1afe4] transition-colors">
            Socials
          </a>
          <a href="#" className="text-white text-sm hover:text-[#d1afe4] transition-colors">
            Docs
        ...
        >
          ...
          <span className="ml-1">
            Refer for a Free Pro <span className="text-[#696969]">(Coming Soon)</span>
          </span>
        </a>
      </div>
      ...
      <div className="flex items-center">
        ...
        <div className="hidden md:flex items-center text-white text-sm mr-4 bg-[#262626] px-3 py-1.5 rounded-md">
          ...
          <CopyButton text={contractAddress} successMessage="Contract address copied!" />
        </div>

        {/* Globe button */}
        <button
          className="flex items-center p-2 bg-transparent rounded-full mr-2 hover:bg-[#262626] transition-colors"
          onClick={() => {
            addToast({
              type: "info",
        ...
        <button
          className="hidden sm:flex px-3 py-1.5 text-[#D1AFE4] text-sm bg-[#262626] rounded-md mr-2 border border-[#3F3F3F] hover:bg-[#3f3f3f] transition-colors"
          onClick={() => {
        ...
        {/* Telegram button */}
        <button
          className="hidden sm:flex px-3 py-1.5 text-white font-medium text-sm rounded-md hover:opacity-90 transition-colors"
          style={{
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden ml-2 p-2 text-white rounded-md hover:bg-[#262626]"
